  ...
  providers: [
    {
      ...
      useExisting: forwardRef(() => AuthAdminComponent),
      multi: true,
    }
  ]
})
export class AuthAdminComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    && localStorage.getItem('auth-token') && sessionStorage.getItem("IsLoggedIn") == "true") {
    ...
    if(this.otp.length == this.configOptions.length) {
      ...
      // Configuration of keyVerifiation form
        ...
        this.authService.verifyKey(this.keyVerification.value).subscribe((data:any) => {
          ...
          this.tokenStorage.saveAdmin(true);
          console.log(data);
          if(data.status == 200){
            Swal.fire({
  ...
  validateToken(){
    if(localStorage.getItem('auth-token')){
      if(this.token_auth){
        this.tokenStorage.validateTokenAdmin(this.token_auth).subscribe(
          (isExpired) => {
            if(isExpired){
            //token is invalid
